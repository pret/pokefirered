...
static void PutAiPartyText(struct BattleDebugMenu *data)
{
    ...
    for (i = 0; i < count; i++)
    {
        if (aiMons[i].wasSentInBattle)
        {
            ...
            *txtPtr++ = CHAR_SPACE;
            if (aiMons[i].gender == MON_MALE)
                ...
                *txtPtr++ = CHAR_FEMALE;
            *txtPtr = EOS;
            AddTextPrinterParameterized5(data->aiMovesWindowId, FONT_SMALL, text, i * 41, 0, 0, NULL, 0, 0); // TODO: font FONT_SMALL_NARROW
        }
        ...
        *txtPtr = EOS;
        AddTextPrinterParameterized5(data->aiMovesWindowId, FONT_SMALL, text, i * 41, 15, 0, NULL, 0, 0); // TODO: font FONT_SMALL_NARROW

        for (j = 0; j < MAX_MON_MOVES; j++)
        {
            txtPtr = StringCopyN(text, GetMoveName(aiMons[i].moves[j]), 8);
            *txtPtr = EOS;
            AddTextPrinterParameterized5(data->aiMovesWindowId, FONT_SMALL, text, i * 41, 35 + j * 15, 0, NULL, 0, 0); // TODO: font FONT_SMALL_NARROW
        }
        ...
        *txtPtr = EOS;
        AddTextPrinterParameterized5(data->aiMovesWindowId, FONT_SMALL, text, i * 41, 35 + (j + 1) * 15, 0, NULL, 0, 0); // TODO: font FONT_SMALL_NARROW
    }
