...
static const u8 sArrowDown_Gfx[] = INCBIN_U8("graphics/misc/reset_rtc_screen_downarrow.4bpp");
static const u8 sArrowRight_Gfx[] = INCBIN_U8("graphics/misc/reset_rtc_screen_rightarrow.4bpp");
static const u16 sArrow_Pal[] = INCBIN_U8("graphics/misc/reset_rtc_screen_arrow.gbapal");
...
static void Task_ShowResetRtcPrompt(u8 taskId)
{
    ...
    switch (tState)
    {
        ...
        case 1:
            if (JOY_NEW(B_BUTTON))
            {
                // Cancel, exit without resetting RRTC
                DestroyTask(taskId);
                DoSoftReset();
            }
