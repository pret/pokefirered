...
// Alias gBackupMapData to avoid using heap.
struct BattleTestRunnerState *const gBattleTestRunnerState = (void *)gBackupMapData;
STATIC_ASSERT(sizeof(struct BattleTestRunnerState) <= sizeof(gBackupMapData), gBackupMapDataSpace);
...
static void BattleTest_SetUp(void *data)
{
    ...
    if (sizeof(*STATE) + test->resultsSize * STATE->parameters > sizeof(gBackupMapData))
        Test_ExitWithResult(TEST_RESULT_ERROR, "OOM: STATE (%d) + STATE->results (%d) too big for gBackupMapData (%d)", sizeof(*STATE), test->resultsSize * STATE->parameters, sizeof(gBackupMapData));
    STATE->results = (void *)((char *)gBackupMapData + sizeof(struct BattleTestRunnerState));
...
void UseItem(u32 sourceLine, struct BattlePokemon *battler, struct ItemContext ctx)
{
    s32 i;
    s32 battlerId = battler - gBattleMons;
    bool32 requirePartyIndex = ItemId_GetType(ctx.itemId) == ITEM_TYPE_PARTY_MENU || ItemId_GetType(ctx.itemId) == ITEM_TYPE_PARTY_MENU_MOVES;
    ...
    if (ItemId_GetType(ctx.itemId) == ITEM_TYPE_PARTY_MENU_MOVES)
    {
