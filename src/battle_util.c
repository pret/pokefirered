...
// TODO: remove when battle_ai_util
    ...
    do
    {
        ...
        case ENDTURN_SNOW:
            if (gBattleWeather & B_WEATHER_SNOW)
            {
                ...
                gBattleScripting.animArg1 = B_ANIM_HAIL_CONTINUES; // B_ANIM_SNOW_CONTINUES; // TODO: Animation
            ...
            if (gBattleMons[battler].status2 & STATUS2_UPROAR)
            {
                for (gBattlerAttacker = 0; gBattlerAttacker < gBattlersCount; gBattlerAttacker++)
                {
                     ...
                     && GetBattlerAbility(gBattlerAttacker) != ABILITY_SOUNDPROOF)
                    {
                        ...
                        BtlController_EmitSetMonData(battler, BUFFER_A, REQUEST_STATUS_BATTLE, 0, 4, &gBattleMons[gBattlerAttacker].status1);
    ...
    case ABILITYEFFECT_ON_SWITCHIN: // 0
        ...
        case ABILITY_HOSPITALITY:
             ...
             && IsBattlerAlive(partner))
            {
                ...
                gBattleMoveDamage = (gBattleMons[partner].maxHP / 4) * -1;
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_CURSED_BODY:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             /* && !IsDynamaxed(gBattlerAttacker) */ // TODO: Dynamax // TODO: Max Moves don't make contact, useless?
             && (Random() % 3) == 0)
            {
        ...
        case ABILITY_ANGER_POINT:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && CompareStat(battler, STAT_ATK, MAX_STAT_STAGE, CMP_LESS_THAN))
            {
                SET_STATCHANGER(STAT_ATK, (MAX_STAT_STAGE - gBattleMons[battler].statStages[STAT_ATK]), FALSE); // TODO: check if (MAX_STAT_STAGE - gBattleMons[battler].statStages[STAT_ATK]) should be in brackets
                BattleScriptPushCursor();
...
static u8 ItemHealHp(u32 battler, u32 itemId, bool32 end2, bool32 percentHeal)
{
        ...
        && HasEnoughHpToEatBerry(battler, 2, itemId))
    {
        ...
        if (gBattleResources->flags->flags[battler] & RESOURCE_FLAG_EMERGENCY_EXIT
         && gBattleMons[battler].hp >= gBattleMons[battler].maxHP  / 2) /* GetNonDynamaxHP(battler) >= GetNonDynamaxMaxHP(battler)  / 2) */ // TODO: Dynamax
            gBattleResources->flags->flags[battler] &= ~RESOURCE_FLAG_EMERGENCY_EXIT;
...
u8 ItemBattleEffects(u8 caseID, u32 battler, bool32 moveTurn)
{
    ...
    case ITEMEFFECT_ON_SWITCH_IN:
        if (!gSpecialStatuses[battler].switchInItemDone)
        {
            ...
            if (effect != 0)
            {
                ...
                gActiveBattler = gBattlerAttacker = gPotentialItemEffectBattler = gBattleScripting.battler = battler;
                switch (effect)
                {
    ...
    case ITEMEFFECT_USE_LAST_ITEM:
        ...
        if (effect)
        {
            gActiveBattler = gPotentialItemEffectBattler = gBattleScripting.battler = battler;
